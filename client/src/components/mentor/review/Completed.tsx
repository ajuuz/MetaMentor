...
import type {  ReviewForMentor } from '@/types/reviewTypes'
...
const resultSchema = z.object({
  ...
  status: z.enum([
    REVIEW_STATUS.PASS,
    REVIEW_STATUS.FAIL,
  ]) as z.ZodType<Extract<ReviewStatus, 'pass' | 'fail'>>,
})
    ...
    if (data.status === REVIEW_STATUS.FAIL) {
      if (data.theory >= 5 && data.practical >= 5) {
        ctx.addIssue({
          path: ["status"],
          code: "custom",
          message: "Either theory or practical must be below 5 when status is FAIL",
        });
      }
    }
  })
...
const Completed = ({ review, start, end }: Props) => {
  ...
  const { register, handleSubmit, setValue, formState: { errors } } = useForm<ResultForm>({
      ...
      <AnimatePresence>
        {feedBackPopupToggle &&
          <motion.div
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="w-[70%] p-5 flex flex-col gap-4 items-center">
              ...
              <div className='w-full'>
                ...
                <Textarea disabled={review.status !== REVIEW_STATUS.PENDING} {...register('feedBack')} className="border-2 bg-white min-h-50 max-h-70" placeholder="Enter your feedback here..." />
              ...
              <div className='w-full flex flex-col sm:flex-row gap-5 justify-between'>
                <div className='flex gap-2'>
                  ...
                  <Input type='number' {...register("theory", { valueAsNumber: true })} placeholder='0-10' />
                </div>
                <div className='flex gap-2'>
                  <Label className='text-nowrap flex-1'>Practical :</Label>
                  <Input type='number' {...register("practical", { valueAsNumber: true })} placeholder='0-10' />
                </div>
              </div>
              <div className='w-full'>
                {errors.theory && <p className="text-red-500">*{errors.theory.message}</p>}
                {errors.practical && <p className="text-red-500">*{errors.practical.message}</p>}
      ...
      <CardContent className="flex flex-col items-center justify-center h-full space-y-6">
        ...
        <div className="bg-slate-50 rounded-lg p-6 w-full max-w-md">
          ...
          <div className="space-y-2 text-sm text-slate-700">
            ...
            <p>• Payment: {review.payment.status} <span className="text-blue-600">( Method : {review.payment.method} )</span> <span className="text-green-600">Amount : {review.commissionAmount+review.mentorEarning}</span></p>
          </div>
        </div>
