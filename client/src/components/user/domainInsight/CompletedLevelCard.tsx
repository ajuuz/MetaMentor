...
const CompletedLevelCard = ({
  review,
  index,
}: {
  review: StudentReviewCard;
  index: number;
}) => {
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <div className="text-gray-600 ">
            ...
            <div className="flex flex-wrap gap-2 font-semibold">
              <p className="text-muted-foreground bg-slate-100 p-1 px-2 rounded">
                Date: {dateFormatter(new Date(review.slot.start))}
              </p>
              <p className="text-muted-foreground bg-slate-100 p-1 px-2 rounded">
                Day: {new Date(review.slot.start).getDay()}
              </p>
              <p className="text-muted-foreground bg-slate-100 p-1 px-2 rounded">
                Start At:{" "}
                {new Date(review.slot.start).toLocaleTimeString()}
              </p>
              <p className="text-muted-foreground bg-slate-100 p-1 px-2 rounded">
                End At: {new Date(review.slot.end).toLocaleTimeString()}
              </p>
            </div>
          </div>
          ...
          <div className="flex justify-between gap-2">
            <div className="flex flex-col justify-center items-center border-2 border-black flex-1 bg-black/10 rounded-lg">
              ...
              <span className="font-semibold capitalize text-lg">
                Rs. {review.commissionAmount + review.mentorEarning}
              </span>
            </div>
            <div
              className={`text-gray-600 bg-blue-100 p-1 px-2 rounded ${
                review.status === "pass"
            ...
            >
              ...
              <div className="flex justify-between gap-3">
                <div
                  className={`bg-blue-50 p-1 border-2 px-2 rounded ${
                    review.status === "pass"
            ...
            <ContentViewerModal
              triggerer={
                <Button
                  size="sm"
                  variant="outline"
                  className={`gap-2 cursor-pointer rounded-full px-4 py-2 text-sm ${
                    review.status === "pass"
