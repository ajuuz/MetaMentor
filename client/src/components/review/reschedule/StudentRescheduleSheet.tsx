...
import { rescheduleReviewByStudent } from "@/services/userService/reviewApi";
...
const StudentRescheduleSheet = ({
  ...
  const slotQuery = useGetSlotsForStudQuery(mentorId, selectedDay);
  const reviewQuery = useGetSlotReviewsForStudentQuery(mentorId, selectedDate!);
  ...
  const { mutate: rescheduleMutation } = useMutation({
    mutationFn: rescheduleReviewByStudent,
    onSuccess: (response) => {
  ...
  const handleRescheduleSubmit = (start:string|undefined,end:string|undefined) => {
    if(studentText.trim() && start && end){
      const slot={start,end}
      rescheduleMutation({studentText,reviewId,mentorId,slot})
    }
  };
              ...
              <CardContent className="space-y-6">
                <div className="flex flex-col justify-between text-center gap-4 text-white font-serif">
                  ...
                  <div className="flex-1 rounded-xl p-4">
                    <Textarea className="text-black" onChange={(e)=>setStudentText(e.target.value)}/>
                    {selectedSlots.length > 0 && (
                      <div className="mt-6 flex justify-center">
                        <Button onClick={()=>handleRescheduleSubmit(start,end)}>Submit</Button>
                      </div>
                    )}
                  </div>
                </div>
...
export default StudentRescheduleSheet;
