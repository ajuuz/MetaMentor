...
import { useGetSlotsForMentQuery } from "@/hooks/tanstack/slot";
import { useGetSlotReviewsForMentQuery } from "@/hooks/tanstack/review";
...
import { slotValidityChecker } from "@/services/mentorService.ts/slotApi";
import { Textarea } from "@/components/ui/textarea";
import { rescheduleReviewByMentor } from "@/services/mentorService.ts/reviewApi";
...
const MentorRescheduleSheet = ({
  ...
  const slotQuery = useGetSlotsForMentQuery(selectedDay);
  const reviewQuery = useGetSlotReviewsForMentQuery(selectedDate!);
  ...
  const { mutate: slotValidityCheckerMutation } = useMutation({
    ...
    const { mutate: rescheduleMutation } = useMutation({
      mutationFn: rescheduleReviewByMentor,
      onSuccess: (response) => {
  ...
  const handleSelectSlot = (slotId: string) => {
    ...
    if (selectedSlots.length === 0) {
      slotValidityCheckerMutation({ date: selectedDate!, slotId });
  ...
  const handleRescheduleSubmit = (
    ...
    if (mentorText.trim() && start && end) {
      const slot = { start, end };
      rescheduleMutation({ mentorText, reviewId, slot });
    }
  };
                      ...
                      (slot) =>
                        slot.enabled && (
                          <div
                            key={slot._id}
                            onClick={() =>
                              !slot.isBooked && handleSelectSlot(slot._id)
                            }
                  ...
                  <div className="flex-1 rounded-xl p-4">
                    ...
                    {selectedSlots.length > 0 && (
                      <div className="mt-6 flex justify-center">
                        <Button
                          onClick={() => handleRescheduleSubmit(start, end)}
                        >
                          Submit
                        </Button>
                      </div>
...
export default MentorRescheduleSheet;
