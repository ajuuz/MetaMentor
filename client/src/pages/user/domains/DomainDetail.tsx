...
const DomainDetail = () => {
  ...
  const handleEnroll = () => {
    if (domainId) enrollDomainMutation(domainId);
    navigate("/dashboard");
  };
      ...
      <AnimatePresence>
        {level && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="rounded-lg fixed top-1/2 -translate-y-1/2 left-1/2 -translate-x-1/2 border w-4/5 h-4/5 bg-white flex flex-col items-center gap-5 p-6"
          >
            <X
              onClick={() => setLevel(null)}
              className="absolute right-6 bg-black text-white"
            />
