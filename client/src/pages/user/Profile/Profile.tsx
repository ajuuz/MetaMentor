...
const Profile = () => {
  ...
  const [errors, setErrors] = useState<
    Pick<UserDetailsType, "name" | "mobileNumber">
  >({
    name: "",
    mobileNumber: "",
  });
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.value === "" && e.target.name==='mobileNumber') {
      setUserDetails((prev) => ({ ...prev, [e.target.name]: null}));
    } else {
  ...
  const validation = () => {
    ...
    if (!userDetails.name.trim()) {
      ...
      setErrors((prev) => ({
        ...prev,
        name: "Name must contain only letters and at least 3 characters. format",
      }));
      error = true;
    }
    if (
      userDetails.mobileNumber &&
      /^\d{10}$/.test(userDetails.mobileNumber) === false
    ) {
      setErrors((prev) => ({
        ...prev,
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      let payload: Partial<typeof userDetails> = {};

      Object.keys(userDetails).forEach((key) => {
        const typedKey = key as keyof typeof userDetails;
        if (userDetails[typedKey] !== profileData?.[typedKey]) {
          payload[typedKey] = userDetails[typedKey]!;
        }
      });
    ...
    <div className="flex flex-col gap-5 px-5 z-1 w-[450px]  relative ">
      ...
      <div className="flex items-center gap-3">
        {isViewMode && userDetails.profileImage ? (
          <div className="bg-black/30 border-5 text-white rounded-[50%] p-3 text-[0.6rem] relative flex justify-center font-medium">
            <img
              ...
              alt={`${userDetails.name}'s profile pic`}
            />
          </div>
        ) : !isViewMode ? (
          <Label
            htmlFor="profilePic"
            className={`bg-black/30 border-5 text-white rounded-[50%] ${
        ...
        <div className={`${userDetails.profileImage && "flex flex-col gap-5"}`}>
          <p
            className={`${
              userDetails.profileImage ? "text-3xl" : "text-4xl"
            } font-medium`}
          >
      ...
      <div className="grid grid-cols-2 gap-5">
        ...
        <div className="flex flex-col gap-2">
          ...
          <SelectComponent
            disabled={isViewMode}
            selectKey="country"
            placeHolder={userDetails?.country || "Country"}
        ...
        <div className="flex flex-col gap-2 ms-4">
          ...
          <SelectComponent
            disabled={isViewMode}
            selectKey="gender"
            placeHolder={userDetails?.gender || "gender"}
            handleSelectChange={handleSelectChange}
