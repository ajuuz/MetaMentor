...
import {
  getDomainsNameAndId,
  registerForm,
} from "@/services/mentorService.ts/registrationApi";
import { useNavigate, useParams } from "react-router-dom";
import {
  acceptMentorApplication,
  getSpecificMentor,
} from "@/services/adminService.ts/mentorApi";
import {
...
export default function MentorDetailsManage() {
  ...
  const [userDetails, setUserDetails] = useState<
    Omit<UserDetailsType, "_id" | "role" | "isVerified">
  >({
    name: "",
    email: "",
    country: null,
  ...
  const {
    mutate: mentorRegisterVerifyMutation,
      ...
      <Card className="p-6">
        <CardContent className="grid grid-cols-1  md:grid-cols-2 gap-10">
                  ...
                  <div className=" border flex flex-wrap p-5 gap-5 h-full">
                    {domains.map((domain, index) => (
                      <div
                        ...
                        className="cursor-pointer py-2 px-3  bg-black text-white rounded-lg"
                      >
                        {domain.name}
                      </div>
                    ))}
                ...
                <div className="border flex flex-wrap p-5 gap-5 h-full">
                  {selectedDomains.map((domain, index) => (
                    <div className="cursor-pointer py-2 px-3 relative bg-black text-white rounded-lg">
                      {!purpose.adminVerification && (
                        <div
                          onClick={() => handleRemoveSelectedDomain(index)}
                          className="scale-75 -right-2 -top-2 bg-red-500 rounded-4xl absolute"
                        >
                ...
                <div className="border-2 p-2 rounded-lg relative">
                  ...
                  {!purpose.adminVerification && (
                    <Cross
                      onClick={() =>
                        setWorkedAt((prev) =>
                          prev.filter((_, ind) => index !== ind)
                        )
                      }
                      className="rotate-45 top-2 right-2  text-white bg-red-500 rounded-4xl absolute p-1"
                    />
                  )}
                </div>
              ))}
          ...
          {purpose.mentorRegister ? (
            <InputImageComponent
              containerDivStyle=" col-span-2  grid grid-cols-1 md:grid-cols-2 gap-6 mt-4"
              images={images as (Blob | null)[]}
              setImages={
                setImages as React.Dispatch<
                  React.SetStateAction<(null | Blob)[]>
                >
              }
              labels={["Add CV", "Add Verification Certificate"]}
            />
          ) : (
        ...
        <Button
          ...
          onClick={
            purpose.mentorRegister
              ? handleSubmit
              : () => handleVerify(MENTOR_APPLICATION_STATUS.ACCEPTED)
          }
        >
        ...
        {purpose.adminVerification && (
          <Button onClick={() => setIsRejecting(true)}>Reject</Button>
        )}
            ...
            >
              <div className="w-[70%] p-5 flex flex-col gap-4 items-center">
                ...
                <Button
                  ...
                  className="w-full"
                  onClick={() =>
                    handleVerify(MENTOR_APPLICATION_STATUS.REJECTED)
                  }
                >
