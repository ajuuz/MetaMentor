...
const UpcomingReviews = () => {
  ...
  const [pendingReviewState, setPendingReviewState] =
    useState<PENDING_REVIEW_STATE>("notOver");
  ...
  const handleSelectChange = (selectKey: string, value: string) => {
    if (selectKey) setDateRange(value as DATE_RANGE);
  };
          ...
          <Switch
            onClick={() =>
              setPendingReviewState((prev) =>
                prev === "over" ? "notOver" : "over"
              )
            }
            checked={pendingReviewState === "over"}
          />
        </div>
      </div>
      ...
      <div onClick={() => console.log(pendingReviewState)} className="h-full">
        {reviews.map((review) => (
          <ReviewCard
            key={review._id}
            review={review}
            isNotOver={pendingReviewState === "notOver"}
          />
        ))}
      </div>
      <div className="mt-10">
