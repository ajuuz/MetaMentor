...
import {
  updateSlot,
  updateSlotStatus,
} from "@/services/mentorService.ts/slotApi";
import { toast } from "sonner";
import type { DayOfWeekType, DaysType, WeekSlotsType } from "@/types/slotTypes";
import { useMentorGetSlotsQuery } from "@/hooks/slot";

const daysOfWeek: DaysType[] = [
  "Monday",
  "Tuesday",
  "Wednesday",
  "Thursday",
...
const SlotManage = () => {
  ...
  const [activeDay, setActiveDay] = useState<DaysType>(daysOfWeek[0]);
  const [timeRanges, setTimeRanges] =
    useState<Record<string, { startTime: string; endTime: string } | null>>(
      timePeriods
    );
  ...
  const handleToggle = (
    ...
    setSlots((prev) => ({
      ...
      [day]: prev[day].map((slot, i) =>
        i === idx ? { ...slot, enabled: !slot.enabled } : slot
      ),
    }));
  };
  ...
  return (
    <div className="min-h-screen bg-[#f7f7f7] py-10">
      <div className="max-w-3xl mx-auto">
            ...
            {daysOfWeek.map((day) => (
              <Button
                ...
                className={`rounded-full px-6 py-2 font-semibold ${
                  activeDay === day ? "bg-[#d90429] text-white" : ""
                }`}
              ...
              {slots[activeDay].map((slot, idx) => (
                ...
                >
                  ...
                  <Switch
                    ...
                    onCheckedChange={() =>
                      handleToggle(activeDay, idx, slot._id, !slot.enabled)
                    }
                  />
                </div>
              ))}
        ...
        <Button onClick={handleSlotCreation} className="w-full mt-2">
          ADD YOUR SLOT
        </Button>
      </div>
    </div>
