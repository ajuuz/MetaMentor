...
const ManageStudents = () => {
  ...
  const [isPremium, setIsPremium] = useState<string[]>([
    searchParams.get("isPremium") || "",
  ]);
  const { data: getAllStudentsResponse } = useAdminGetAllStudentsQuery(
    currentPage,
    limit,
    sortBy,
    searchTerm,
      ...
      const transformedDetails: TableDetailsType[] = students.map((student) => {
        return {
          ...
          content: [
            ...
            student.country,
            0,
            <Badge variant={student.isBlocked ? "destructive" : "outline"}>
              {student.isBlocked ? "Blocked" : "Active"}
            </Badge>,
            <AlertDialogComponent
              alertTriggerer={<Switch checked={student.isBlocked} />}
              alertDescription="This action cannot be undone. This will permanently delete your
                    account and remove your data from our servers."
              handleClick={() =>
                handleStatusChange(student.userId, !student.isBlocked)
              }
            />,
            student.point,
          ],
  ...
  }, [currentPage,limit, searchTerm, sortBy, isPremium, setSearchParams]);
