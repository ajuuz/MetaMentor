...
export const getDomainSlots = async (
  domainId: string
): Promise<{
  domainSlots: DomainSlotsResponseDTO[];
  bookedSlots: GetDomainReviewSlotResponseDTO[];
}> => {
  try {
...
export const getSlotsForStud = async (
...
export const slotValidityChecker = async ({
  ...
  try {
    const response = await userAxiosInstance.post(
      `/slots/${mentorId}/${date.toISOString()}/${slotId}`
    );
    return response.data;
  } catch (error: any) {
    throw error?.response?.data || error;
