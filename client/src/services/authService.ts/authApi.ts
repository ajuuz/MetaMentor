...
export const signup = async (
  formData: Omit<AuthFormType, "confirmPwd">
...
export const login = async ({
  ...
  email: string;
  password: string;
}): Promise<Required<ApiResponseType<UserType>>> => {
  try {
...
export const googleAuth = async (
  idToken:string): Promise<Required<ApiResponseType<UserType>>> => {
  try {
    const response = await authAxiosInstance.post("/googleAuth", {
      idToken,
    });
    return response.data;
  } catch (error: any) {
...
export const forgotPasswordSendMail = async (
  email: string
): Promise<MutationApiResponse> => {
  try {
    const response = await authAxiosInstance.post("/forgotPassword/mail", {
      email,
    });
    return response.data;
  } catch (error: any) {
...
export const resetPassword = async ({
  ...
  try {
    const response = await authAxiosInstance.patch("/forgotPassword/reset", {
      password,
      token,
    });
    return response.data;
