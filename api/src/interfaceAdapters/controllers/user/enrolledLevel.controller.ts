...
@injectable()
export class UserEnrolledLevelController implements IUserEnrolledLevelController{
    ...
    async saveLevelAssignments(req:Request,res:Response):Promise<void>{
        const {enrolledLevelId,assignments}:SaveAssignmentReqDTO=req.verifiedData;
        await this._saveLevelAssignmentUsecase.execute(enrolledLevelId,assignments)
        res.status(HTTP_STATUS.OK).json({success:true,message:SUCCESS_MESSAGE.LEVEL.SUBMIT_ASSIGNMENT})
    }
}