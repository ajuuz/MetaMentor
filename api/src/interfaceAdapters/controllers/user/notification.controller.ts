...
export class UserNotificationController implements IUserNotificationController{
    ...
    async getNotifications(req:Request,res:Response):Promise<void>{
        ...
        const notifications=await this._getNotificationUsecase.execute(userId,filter!)
    ...
    async markAsRead(req:Request,res:Response):Promise<void>{
        ...
        res.status(HTTP_STATUS.OK).json({success:true,message:'All notification read successfully'});
    }
}