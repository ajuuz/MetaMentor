...
export class TokenRefreshingUsecase implements ITokenRefreshingUsecase{
    ...
    execute(token:string):string{
        if(!token) throw new AuthError(HTTP_STATUS.UNAUTHORIZED,ERROR_MESSAGE.UNAUTHORIZED_ACCESS_NOT_LOGIN);
        const user:JwtPayload=this._tokenService.verifyRefreshToken(token)
        const id= user.id;
        const email= user.email as string;
        const role= user.role as ROLES;
        const accessToken=this._tokenService.generateAccessToken({id,email,role})
        return accessToken
    }
}